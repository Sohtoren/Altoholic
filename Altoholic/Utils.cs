...
namespace Altoholic
{
    ...
    internal abstract class Utils
    {
        ...
        public static uint[] BOZJA_RANKS = [0, 300, 1100, 2200, 3800, 9400, 18000, 32000, 51300, 78000, 106000, 157400, 252000, 416000, 703200, 1124000, 1930000, 2790000, 4435000, 6163000, 8663000, 11471000, 15602000, 20516000, 25870000];
        public static uint[] EUREKA_ELEMENTAL_LEVELS = [1000, 2300, 5060, 10626, 21252, 36128, 46967, 51664, 57088, 63368, 70655, 79134, 89026, 100599, 114180, 131037, 157568, 204839, 286774, 315452, 602513, 668789, 745700, 835184, 939582, 1061728, 1205061, 1373769, 1572966, 1808911, 2170693, 2821901, 4035318, 6254743, 6880217, 7568239, 8336415, 9186730, 10128370, 11171592, 12327851, 13609948, 15032187, 16610567, 18362982, 21117429, 25340915, 32943190, 46120466, 50732512, 55805763, 61497951, 67893738, 75090474, 83200246, 92352273, 102695727, 128369659, 183568612, 275352919];
            ...
            IDalamudTextureWrap? fallbackTexture, IDalamudTextureWrap? emptySlot)
        {
            ...
            else
            {
                ...
                if (g == null) return;
                (string name, string transientOrDescription, IDalamudTextureWrap? icon) = Helpers.Reward.GetCollectibleNameTransientDescriptionIcon(currentLocale, globalCache, Helpers.CharacterCollectible.Glass, g.Id);
                DrawIcon(icon, iconSize);
                if (ImGui.IsItemHovered())
                {
                    Helpers.Reward.DrawCollectibleTooltip(currentLocale, ref globalCache, id, icon, name, transientOrDescription);
                }
            }
        }
        ...
        */
        ...
        public static bool IsNowInTuesdayToFridayWindow()
        {
            DateTime utcNow = DateTime.UtcNow;

            return utcNow switch
            {
                { DayOfWeek: DayOfWeek.Wednesday } or { DayOfWeek: DayOfWeek.Thursday }
                    or { DayOfWeek: DayOfWeek.Tuesday, Hour: > 8 }
                    or { DayOfWeek: DayOfWeek.Friday, Hour: < 8 } => true,
                _ => false
            };
        }
